...
import 'modifier_prefecture_screen_model.dart';
export 'modifier_prefecture_screen_model.dart';

class ModifierPrefectureScreenWidget extends StatefulWidget {
  const ModifierPrefectureScreenWidget({
    super.key,
  ...
  @override
  State<ModifierPrefectureScreenWidget> createState() =>
      _ModifierPrefectureScreenWidgetState();
}

class _ModifierPrefectureScreenWidgetState
    extends State<ModifierPrefectureScreenWidget> {
  late ModifierPrefectureScreenModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => ModifierPrefectureScreenModel());

    _model.mPrefectureTextController ??=
        TextEditingController(text: widget.prefecture?.prefecture);
    _model.mPrefectureFocusNode ??= FocusNode();

    _model.mSprefectureTextController ??=
        TextEditingController(text: widget.prefecture?.sousPref);
    _model.mSprefectureFocusNode ??= FocusNode();

    _model.mIndictatifTextController ??=
        TextEditingController(text: widget.prefecture?.indicatif);
    _model.mIndictatifFocusNode ??= FocusNode();

    _model.prefectureNameTextController ??=
        TextEditingController(text: widget.prefecture?.prefecture);
    _model.prefectureNameFocusNode ??= FocusNode();

    _model.sousPrefTextController ??=
        TextEditingController(text: widget.prefecture?.sousPref);
    _model.sousPrefFocusNode ??= FocusNode();

    _model.indicatifTextController ??=
        TextEditingController(text: widget.prefecture?.indicatif);
                        ...
                        children: [
                          Padding(
                            padding: const EdgeInsetsDirectional.fromSTEB(
                                0.0, 0.0, 0.0, 20.0),
                            child: Text(
                              'Nb : S\'il ny\'a pas de sous-prefecture,  saissisez encore la prefecture dans le champ de sous-prefecture. S\'il y\'a plusieurs sous-prefectures pour une prefecture, vous ajouterez les sous-prefectures un par un.',
                          ...
                          FFButtonWidget(
                            onPressed: () async {
                              await widget.prefecture!.reference
                                  .update(createPrefecturesRecordData(
                                prefecture:
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    '${_model.mPrefectureTextController.text} ajoutée avec succès',
                                    style: TextStyle(
                          ...
                          FFButtonWidget(
                            onPressed: () async {
                              await widget.prefecture!.reference
                                ...
                                sousPref: widget.prefecture?.sousPref,
                                indicatif: widget.prefecture?.indicatif,
                              ));
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
